...
def plot_results(x, y, distance, text, time, color):
    plt.plot(x, y, linestyle='--', marker='.', color=color)
    plt.plot(x[0], y[0], marker='o', color='brown')
    plt.title(text + " Distance: " + str(round(distance, 3)) + " Time: " + str(time) + "s")
    plt.show()
...
def brute_force():
    ...
    _time = round((time() - time_start), 5)
    # print("Shortest Route: ")
    # print(min_route)
    print("Distance: " + str(round(min_distance, 3)) + " Found in: " + str(_time) + "s")
    ...
    plot_results(x, y, min_distance, "Route by Brute-Force.", _time, 'orange')
...
def nearest_neighbour():
    ...
    print("Distance: " + str(round(distance, 3)) + " Found in: " + str(_time) + "s")

    plot_results(x, y, distance, "Route by nearest Neighbour", _time, 'g')
...
def held_karp(dists):
    ...
    plot_results(x, y, opt, "Route by Held-Karp algorithm", _time, 'r')

    print("Cost: " + str(round(opt, 3)) + " Time: " + str(_time) + "s")
...
def display_solution(manager, routing, solution, duration):
    ...
    plan_output = 'Distance: {} '.format(solution.ObjectiveValue() / 1000) + " Found in: " + str(duration) + "s"
    print(plan_output)

    plot_results(x, y, solution.ObjectiveValue() / 1000, "Route by Google OR-Tools.", duration, 'b')
